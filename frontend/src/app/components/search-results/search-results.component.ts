    ...
    @if (results.length > 0) {
      <div class="absolute top-full left-0 right-0 mt-1 bg-[var(--card-bg)] rounded-lg shadow-lg border border-[var(--border-color)] max-h-96 overflow-y-auto z-50">
        @for (result of results; track result.item.id) {
          <div class="p-3 hover:bg-secondary-bg cursor-pointer border-b border-[var(--border-color)] last:border-b-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="text-text-primary font-medium">{{ result.item.name }}</div>
                <!-- <div class="text-text-secondary text-sm">{{ result.item.path }}</div> -->
              </div>
            </div>
          </div>
        }
