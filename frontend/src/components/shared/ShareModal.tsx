...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, items, onShare }) => {
  ...
  useEffect(() => {
    if (searchQuery.length > 0) {
      const mockUsers: any = [
        { _id: '1', name: 'Alice', surname: 'Smith', email: 'alice@example.com', avatar: '' },
        { _id: '2', name: 'Bob', surname: 'Johnson', email: 'bob@example.com', avatar: '' },
        { _id: '3', name: 'Charlie', surname: 'Brown', email: 'charlie@example.com', avatar: '' },
      ];
  ...
  const getUserById = (id: string) => users.find(user => user._id === id);
