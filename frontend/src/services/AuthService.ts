...
class AuthService {
  ...
  async login(email: string, password: string): Promise<UserLoginDto | null> {
    ...
    try {
      ...
      const response = await fetch(
        `${Enviroment.API_URL}${this.controller}/login`,
        {
          method: "POST",
  ...
  async register(
    ...
    email: string,
    password: string
  ): Promise< { UserLoginDto : UserLoginDto, privateKey: string, publicKey: string } | null> {
    ...
    try {
      ...
      const response = await fetch(
        `${Enviroment.API_URL}${this.controller}/register`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      sessionStorage.setItem("accesToken", user.accessToken);
      sessionStorage.setItem("refreshToken", user.refreshToken);
      return {UserLoginDto: user, privateKey: privateKey, publicKey: publicKey};
    } catch (error) {
  ...
  async logout(): Promise<void> {
    ...
    const response = await fetch(
      `${Enviroment.API_URL}${this.controller}/logout`,
      {
        method: "POST",
        headers: {
