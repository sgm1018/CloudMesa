 ...
 * CircleTool.ts
 * 
 * Herramienta para crear círculos/elipses.
 */
...
export class CircleTool extends BaseTool {
  ...
  onMouseDown(point: Point, _event: MouseEvent): void {
    ...
    console.log('CircleTool: started at', point);
  }
  ...
  onMouseMove(point: Point, _event: MouseEvent): void {
    ...
    // Calcular radio basado en distancia del punto inicial
    const radius = this.distance(this.startPoint, point);
    this.previewElement.radius = radius;
  }
  ...
  onMouseUp(point: Point, _event: MouseEvent): void {
    ...
    if (this.isClick(this.startPoint, point)) {
      console.log('CircleTool: click too small, cancelled');
      this.cleanup();
      return;
    }
    ...
    if (!this.callbacks) {
      console.error('CircleTool: callbacks not set');
      this.cleanup();
      return;
    }
    ...
    console.log('CircleTool: created circle', finalElement);
  ...
  renderPreview(): JSX.Element | null {
    ...
    return null; // TODO: Implementar con componente CircleElement
  }
