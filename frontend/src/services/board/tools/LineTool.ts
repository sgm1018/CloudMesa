 ...
 * LineTool.ts
 * 
 * Herramienta para crear líneas rectas.
 */
...
export class LineTool extends BaseTool {
  ...
  onMouseDown(point: Point, _event: MouseEvent): void {
    ...
    console.log('LineTool: started at', point);
  }
  ...
  onMouseUp(point: Point, _event: MouseEvent): void {
    ...
    if (this.isClick(this.startPoint, point)) {
      console.log('LineTool: click too small, cancelled');
      this.cleanup();
      return;
    }
    ...
    if (!this.callbacks) {
      console.error('LineTool: callbacks not set');
      this.cleanup();
      return;
    }
    ...
    console.log('LineTool: created line', finalElement);
  ...
  renderPreview(): JSX.Element | null {
    ...
    return null; // TODO: Implementar con componente LineElement
  }
